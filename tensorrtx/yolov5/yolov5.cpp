...
int main(int argc, char** argv) {
    ...
    //std::string wts_name = "";
    std::string engine_name = "./best_1.engine";
    ...
    {
        cv::Mat frame;
        capture >> frame;     //从capture读取数据到frame
        if(frame.empty())
        {
        ...
        // imageProcessing(frame.clone());
        // std::thread processingThread(imageProcessingThread, frame.clone());
        ...
        std::cout<< "infer_time: " << std::chrono::duration_cast<std::chrono::milliseconds>(end - start).count() << "ms" << std::endl;
        ...
        for (int b = 0; b < fcount; b++) {
            ...
            for (size_t j = 0; j < res.size(); j++) {
                ...
                // std::cout << "box.x " << res[j].bbox[0] << std::endl;
                // std::cout << 'r' << r.size() << std::endl;
                // std::cout << "r.x " << r.x <<std::endl;
                // std::cout << "r.y " << r.y <<std::endl;
                std::cout << "r.w " << r.width <<std::endl;
                std::cout << "r.h " << r.height <<std::endl;
